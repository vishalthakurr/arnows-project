...
const Messages = (props) => {
  ...
  const { usermessage, getmessage, deletemessage, showalert, messageupdated } =
    context;
  ...
  const updatemessage = (newEditmessage) => {
    setShowModal(true);
    seteditmessage({
      id: newEditmessage[0]._id,
      emessage: newEditmessage[0].message,
    });
  };
  ...
  return (
    <div>
      ...
      <div>
        <h1 className="text-center my-6 text-base font-semibold font-serif">
          Your Messages Here
        </h1>
      </div>
      <div>
        <section className="text-gray-600 body-font grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 w-full  my-2  relative">
          {usermessage.map((item) => {
            return (
              <div
                key={item._id}
                className="container m-2   shadow-lg  h-auto overflow-hidden shadow-black relative"
              >
                <div className=" mx-2 ">
                  <div className="p-2 ">
                ...
                <div
                  className="absolute  bottom-2  left-14 text-xl cursor-pointer hover:text-red-600 "
                  title="Delete Message"
                >
                  <BsTrash
                    onClick={() => {
                      deletemessage(item._id);
                      showalert("Delete message successfully", "danger");
                    }}
                  />
                </div>
                <div
                  title="Edit Message"
                  className="absolute bottom-2  left-36 text-xl cursor-pointer  hover:text-green-600"
                >
                  <FiEdit
                    onClick={() => {
